...
// ecBlockSpec holds the block structure for one (version, ECC level) pair.
// Sourced programmatically from the well-established python `qrcode` library's
// RS_BLOCK_TABLE (ISO/IEC 18004 Table 9) to avoid manual-transcription errors.
type ecBlockSpec struct {
	NumBlocks1  int
	Total1      int // total codewords per block in group 1 (data+ecc)
	Data1       int // data codewords per block in group 1
	NumBlocks2  int
	Total2      int
	Data2       int
}

// index [version-1][eccLevel] where eccLevel: 0=L 1=M 2=Q 3=H
var ecBlockTable = [40][4]ecBlockSpec{
...
// alignmentPatternPositions[version-1] gives the coordinate list used for both
// row and column placement of alignment pattern centers (empty for version 1).
